...
class Expedia {
   ...
   * @var string
   */
  protected $api_host = 'api.eancdn.com';

  /**
   * @var string
   */
  protected $hotel_api_uri = '/ean-services/rs/hotel/v3/';
  ...
  public function buildUrl($function) {
    return $this->protocol . $this->api_host . $this->hotel_api_uri . $function;
  }

  public function __call($name, $args) {
    return $this->doCall($this->buildUrl($name), $args[0]);
  }
   ...
   * @throws Exception
   */
  public function doCall($url, $options = array()) {
    $ch = curl_init();

    $data = $options + $this->getDefaultRequestOptions();
    ...
    if (FALSE === $result) {
      throw new Exception(array('verboseMessage' => $this->verbose_log));
    }
    ...
    return $this->checkForEanError($response);
  }
   ...
   * @throws Exception
   */
  public function checkForEanError($response) {
